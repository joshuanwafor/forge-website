...
export default function ShareLinks({ title, url }: { title: string; url: string }) {
  ...
  async function copy() {
    try {
      ...
      // Clipboard can be blocked (insecure context, permissions) — the share
      // links below still work, so fail quietly rather than throwing at the user.
    }
  }
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <button
        type="button"
        onClick={copy}
        className="flex h-9 items-center gap-2 rounded-lg border border-white/8 bg-white/[0.03] px-3 text-xs font-medium text-ink-400 transition-colors hover:border-white/20 hover:text-white"
      >
        <svg
          className="h-4 w-4"
          viewBox="0 0 24 24"
